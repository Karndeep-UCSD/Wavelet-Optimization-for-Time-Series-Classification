...
%     [c1,c2,c3,c4,c5,c6,c7,c8] = WMRA_bior6_8(beats(:,100), true);
    ...
    % Get wavelet coefficients for each wavelet
    [cD1,cD2,cD3,cD4,cD5,cD6,cD7,cD8] = myWMRA(beats, HiD, LoD);
     
    % Concatenate wavelet coefficients
    wav_out = [cD1; cD2; cD3; cD4; cD5; cD6; cD7; cD8];    
    
    % Save Data
    filenameWMRA = 'C:\Users\19095\Documents\ECE251C\ECG_data\WMRA_lead2\' + string(recordName) + '_WMRA.mat';
...
function myPCA(wav)
    w = wav - mean(wav,1);
    % Preallocate for Covariance matrices
    Cov = zeros(size(w,1),size(w,1),size(w,2));
...
function [cD1,cD2,cD3,cD4,cD5,cD6,cD7,cD8] = myWMRA(beats, HiD, LoD)
